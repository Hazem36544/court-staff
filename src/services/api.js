...
api.interceptors.request.use(
    (config) => {
        ...
        const token = sessionStorage.getItem('wesal_staff_token');
        if (token) {
...
export const authAPI = {
    ...
    loginFamilyCourt: (creds) => api.post('/api/auth/court/sign-in', creds), // تم التحديث بناءً على السواجر
    ...
    getCurrentUser: () => {
        // ✅ التعديل هنا: استخدام sessionStorage بدلاً من localStorage
        const savedUser = sessionStorage.getItem('wesal_staff_user_data');
...
export const courtAPI = {
    ...
    updateAlimony: (id, data) => api.put(`/api/alimony-schedules/${id}`, data), // تم التحديث وإزالة البارامترات الزائدة
    deleteAlimony: (id) => api.delete(`/api/alimony-schedules/${id}`), // تم التحديث وإزالة البارامترات الزائدة
...
export const lookupAPI = {
    ...
    getLocation: (id) => api.get(`/api/visit-centers/${id}`, { params: { locationId: id } }), // تم التحديث
    createLocation: (data) => api.post('/api/visit-centers', data), // تم التحديث
    updateLocation: (id, data) => api.put(`/api/visit-centers/${id}`, data, { params: { locationId: id } }), // تم التحديث
    deleteLocation: (id) => api.delete(`/api/visit-centers/${id}`, { params: { locationId: id } }), // تم التحديث
};
...
export const visitationAPI = {
    ...
    complete: (id) => api.patch(`/api/visit-sessions/${id}/check-out`), // تم التحديث
