    ...
    >
      ...
      <nav className="flex-1 w-full px-2 flex flex-col gap-3 overflow-y-auto [&::-webkit-scrollbar]:hidden" style={{ scrollbarWidth: 'none' }}>
        {menuItems.map((item) => {
          ...
          // التعديل هنا: تاب العائلات هيفضل منور حتى لو دخلنا جوه تفاصيل القضية
          const isActive =
            currentScreen === item.id ||
            (item.id === 'families-management' && (currentScreen === 'new-family' || currentScreen === 'family-details' || currentScreen === 'case-details'));
