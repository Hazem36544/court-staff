...
export function PerformanceHeader({ onBack, userData, userRole, getRoleTitle }) {
  return (
    <div className="relative w-full bg-[#1e3a8a] rounded-[2rem] p-6 md:p-8 text-white flex items-center justify-between overflow-hidden shadow-xl">
      ...
      <div className="flex items-center gap-4 md:gap-5 relative z-10">
        ...
        <div>
          <h1 className="text-xl md:text-2xl font-bold mb-1">تقرير الأداء الشامل</h1>
          <p className="text-blue-200 text-xs md:text-sm opacity-90 font-bold">
            ملخص إحصائيات الإنجاز لـ: {userData?.fullName} ({getRoleTitle(userRole || userData?.role)})
          </p>
        </div>
      </div>

      <div className="hidden md:flex bg-white/10 p-4 rounded-2xl backdrop-blur-sm border border-white/10 relative z-10">
         <BarChart className="w-8 h-8 text-blue-100" />
      </div>
    </div>
  );
}