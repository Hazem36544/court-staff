...
export default function ComplaintsStats({ counts }) {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <Card className="p-4 bg-white border-blue-100 shadow-[0_4px_20px_rgba(0,0,0,0.03)] hover:shadow-md transition-shadow rounded-[1.5rem]">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-2xl font-black text-blue-600 font-mono">{counts.underReview}</p>
          </div>
      ...
      <Card className="p-4 bg-white border-green-100 shadow-[0_4px_20px_rgba(0,0,0,0.03)] hover:shadow-md transition-shadow rounded-[1.5rem]">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-2xl font-black text-green-600 font-mono">{counts.resolved}</p>
          </div>
      ...
      <Card className="p-4 bg-white border-red-100 shadow-[0_4px_20px_rgba(0,0,0,0.03)] hover:shadow-md transition-shadow rounded-[1.5rem]">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm text-gray-500 font-bold mb-1">مرفوضة / معادة</p>
            <p className="text-2xl font-black text-red-600 font-mono">{counts.rejected}</p>
          </div>
          <div className="w-12 h-12 bg-red-50 rounded-2xl flex items-center justify-center"><XOctagon className="w-6 h-6 text-red-500" /></div>
        </div>
      </Card>
    </div>
  );
