...
const CourtStaffChangePasswordForm = ({
  ...
  <div className="bg-white rounded-2xl shadow-lg p-8 mb-6 animate-in zoom-in-95 duration-300 relative overflow-hidden border-t-4" style={{ borderColor: '#2c5aa0' }}>
    ...
    <div className="space-y-6">
      <div style={{ position: 'absolute', opacity: 0, zIndex: -1, width: '1px', height: '1px', overflow: 'hidden' }} aria-hidden="true">
        <input type="password" name="fake_old_pwd" autoComplete="current-password" tabIndex="-1" />
      </div>
      ...
      <div className="relative">
        <label className="block mb-2 text-[#2c3e50] font-bold text-sm text-right">كلمة المرور الحالية</label>
        <div className="relative">
          <input
            type="text" name="ws_curr_pass" value={password} onChange={handleInputChange(setPassword, 'currentPassword')}
            placeholder="••••••••" autoComplete="off" style={{ WebkitTextSecurity: showPassword ? 'none' : 'disc' }}
      ...
      <div className="relative">
        ...
        <div className="relative">
          <input
            type="text" name="ws_new_pass" value={newPassword} onChange={handleInputChange(setNewPassword, 'newPassword')}
            placeholder="••••••••" autoComplete="off" style={{ WebkitTextSecurity: showNewPassword ? 'none' : 'disc' }}
      ...
      <div className="relative">
        ...
        <div className="relative">
          <input
            type="text" name="ws_conf_pass" value={confirmPassword} onChange={handleInputChange(setConfirmPassword, 'confirmPassword')}
            placeholder="••••••••" autoComplete="off" style={{ WebkitTextSecurity: showNewPassword ? 'none' : 'disc' }}
      ...
      >
        {isLoading ? <><Loader2 className="w-5 h-5 animate-spin" /> جاري الحفظ...</> : <><Lock className="w-4 h-4" /> حفظ وتأمين الحساب</>}
      </button>
    </div>
  </div>
);

export default CourtStaffChangePasswordForm;