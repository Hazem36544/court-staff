...
import { Scale, Briefcase, Calendar } from 'lucide-react';
import { getRoleTitle } from './DashboardHelpers';

export default function DashboardHeader({ userData, userRole, today }) {
  return (
    <div className="relative w-full bg-[#1e3a8a] rounded-[2rem] p-6 md:p-8 text-white flex flex-col md:flex-row justify-between items-center overflow-hidden shadow-xl gap-6">
      ...
      <div className="flex items-center gap-5 relative z-10 w-full md:w-auto">
        ...
        <div>
          <p className="text-blue-200 text-xs md:text-sm font-bold mb-1 opacity-90">مرحباً بك،</p>
          <h1 className="text-xl md:text-3xl font-bold mb-2 tracking-wide">{userData.fullName || 'موظف المحكمة'}</h1>
          <div className="flex items-center gap-3 text-blue-100 text-xs md:text-sm font-bold opacity-90">
            <span className="flex items-center gap-1.5 bg-blue-900/40 px-3 py-1.5 rounded-full border border-blue-800">
              ...
              {getRoleTitle(userRole || userData.role)}
            </span>
          </div>
        </div>
      </div>
